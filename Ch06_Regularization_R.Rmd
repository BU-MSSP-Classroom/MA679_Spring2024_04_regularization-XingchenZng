...
ames_raw$SalePrice <- as.numeric(as.character(ames_raw$SalePrice))
...
predictors <- c("Lot Area", "Overall Qual", "Gr Liv Area", "Garage Area")
target <- "SalePrice"


x_train <- as.matrix(ames_raw_2008[, predictors])
y_train <- as.vector(ames_raw_2008[, target])
x_test <- as.matrix(ames_raw_2009[, predictors])
y_test <- as.vector(ames_raw_2009[, target])


calc_loss <- function(prediction, actual) {
  difpred <- actual - prediction
  RMSE <- sqrt(mean(difpred^2))
  operation_loss <- sum(abs(difpred[difpred < 0])) + sum(0.1 * actual[difpred > 0])
  return(list(RMSE, operation_loss))
}
...
best_lambda_ridge <- ridge_cv$bestTune$lambda
...
best_lambda_lasso <- lasso_cv$bestTune$lambda

elastic_net_cv <- train(
  x = x_train,
  y = y_train,
  method = "glmnet",
  trControl = trainControl("cv", number = 10),
...
best_alpha <- elastic_net_cv$bestTune$alpha
best_lambda_elastic <- elastic_net_cv$bestTune$lambda
...
elastic_net_model <- glmnet(x_train, y_train, alpha = best_alpha, lambda = best_lambda_elastic)
elastic_net_pred <- predict(elastic_net_model, newx = x_test)

ridge_loss <- calc_loss(ridge_pred, y_test)
lasso_loss <- calc_loss(lasso_pred, y_test)
elastic_net_loss <- calc_loss(elastic_net_pred, y_test)
...
ridge_variable_coeffs <- as.vector(ridge_coeffs[-1])

lasso_coeffs <- coef(lasso_model)
lasso_intercept <- lasso_coeffs[[1]]
lasso_variable_coeffs <- as.vector(lasso_coeffs[-1])

elastic_net_coeffs <- coef(elastic_net_model)
elastic_net_intercept <- elastic_net_coeffs[[1]]
elastic_net_variable_coeffs <- as.vector(elastic_net_coeffs[-1])


coefficients_df <- data.frame(
  Variable = c("Intercept", colnames(x_train)),
  Ridge = c(ridge_intercept, ridge_variable_coeffs),
  Lasso = c(lasso_intercept, lasso_variable_coeffs),
  Elastic_Net = c(elastic_net_intercept, elastic_net_variable_coeffs)
)
...
fit_models <- function(data) {
  ridge_coeffs_list <- list()
  lasso_coeffs_list <- list()
  elastic_net_coeffs_list <- list()
  
  for (i in 1:num_bootstrap) {
    bootstrap_sample <- data[sample(nrow(data), replace = TRUE), ]
    
    ridge_model <- glmnet(as.matrix(bootstrap_sample[, predictors]), bootstrap_sample[, target], alpha = 0, lambda = best_lambda_ridge)
    lasso_model <- glmnet(as.matrix(bootstrap_sample[, predictors]), bootstrap_sample[, target], alpha = 1, lambda = best_lambda_lasso)
    elastic_net_model <- glmnet(as.matrix(bootstrap_sample[, predictors]), bootstrap_sample[, target], alpha = best_alpha, lambda = best_lambda_elastic)
  
    ridge_coeffs <- coef(ridge_model)[-1]
    lasso_coeffs <- coef(lasso_model)[-1]
    elastic_net_coeffs <- coef(elastic_net_model)[-1]
    
    ridge_coeffs_list[[i]] <- ridge_coeffs
    lasso_coeffs_list[[i]] <- lasso_coeffs
    elastic_net_coeffs_list[[i]] <- elastic_net_coeffs
  }

  ridge_coeffs_df <- do.call(rbind, ridge_coeffs_list)
  lasso_coeffs_df <- do.call(rbind, lasso_coeffs_list)
  elastic_net_coeffs_df <- do.call(rbind, elastic_net_coeffs_list)
  
  return(list(ridge = ridge_coeffs_df, lasso = lasso_coeffs_df, elastic_net = elastic_net_coeffs_df))
}

coefficients_boot <- fit_models(ames_raw_2008)

average_coeffs <- lapply(coefficients_boot, function(coeffs) apply(coeffs, 2, mean))
sd_coeffs <- lapply(coefficients_boot, function(coeffs) apply(coeffs, 2, sd))
...
test_error <- sqrt(mean((test_predictions - test_data$Apps)^2))
...
test_predictions_ridge <- predict(ridge_model, newx = as.matrix(test_data[, -1]))

test_error_ridge <- sqrt(mean((test_predictions_ridge - test_data$Apps)^2))

print(paste("Ridge Regression Test Error (RMSE):", test_error_ridge))
```
